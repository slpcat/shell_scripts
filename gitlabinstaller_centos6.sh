...
yum -y install which sudo patch gcc-c++ cmake readline-devel zlib-devel libffi-devel openssl-devel make autoconf automake libtool bison libxml2-devel libxslt-devel libyaml-devel
...
sed -i "s/username: git/username: root/g" config/database.yml
...
sed -i "s/source\ \"https\:\/\/rubygems.org\"/source\ \"https\:\/\/ruby.taobao.org\"/g" Gemfile
...
## Install init script
curl --output /etc/init.d/gitlab https://raw.githubusercontent.com/gitlabhq/gitlab-recipes/master/init/sysvinit/centos/gitlab-unicorn
